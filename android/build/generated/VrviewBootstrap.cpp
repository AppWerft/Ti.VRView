...
#define TAG "ti.vrview"

using namespace v8;

static Persistent<Object> bindingCache;

static void Vrview_getBinding(const FunctionCallbackInfo<Value>& args)
{
	Isolate* isolate = args.GetIsolate();
	EscapableHandleScope scope(isolate);

	if (args.Length() == 0) {
		titanium::JSException::Error(isolate, "Vrview.getBinding requires 1 argument: binding");
	...
	titanium::bindings::BindEntry *extBinding = titanium::bindings::VrviewBindings::lookupGeneratedInit(
...
static void Vrview_init(Local<Object> exports, Local<Context> context)
{
	...
	Local<FunctionTemplate> constructor = FunctionTemplate::New(isolate, Vrview_getBinding);
	exports->Set(String::NewFromUtf8(isolate, "getBinding"), constructor->GetFunction(context).ToLocalChecked());
}

static void Vrview_dispose(Isolate* isolate)
{
	HandleScope scope(isolate);
	if (bindingCache.IsEmpty()) {
		return;
	...
	for (uint32_t i = 0; i < length; ++i) {
		...
		titanium::bindings::BindEntry *extBinding =
			titanium::bindings::VrviewBindings::lookupGeneratedInit(*binding, bindingLength);
...
static titanium::bindings::BindEntry VrviewBinding = {
	"ti.vrview",
	Vrview_init,
	Vrview_dispose
};
...
Java_ti_vrview_VrviewBootstrap_nativeBootstrap
	(JNIEnv *env, jobject self)
{
	titanium::KrollBindings::addExternalBinding("ti.vrview", &VrviewBinding);
	titanium::KrollBindings::addExternalLookup(&(titanium::bindings::VrviewBindings::lookupGeneratedInit));
}
