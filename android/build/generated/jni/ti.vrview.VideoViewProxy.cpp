...
, javaClass, nameSymbol);
	...
	// Method bindings --------------------------------------------------------
	titanium::SetProtoMethod(isolate, t, "resume", VideoViewProxy::resume);
	titanium::SetProtoMethod(isolate, t, "destroy", VideoViewProxy::destroy);
	titanium::SetProtoMethod(isolate, t, "pause", VideoViewProxy::pause);
...
// Methods --------------------------------------------------------------------
void VideoViewProxy::resume(const FunctionCallbackInfo<Value>& args)
{
	LOGD(TAG, "resume()");
	...
	if (!methodID) {
		methodID = env->GetMethodID(VideoViewProxy::javaClass, "resume", "()V");
		if (!methodID) {
...
void VideoViewProxy::destroy(const FunctionCallbackInfo<Value>& args)
{
	LOGD(TAG, "destroy()");
	Isolate* isolate = args.GetIsolate();
	HandleScope scope(isolate);
	...
	static jmethodID methodID = NULL;
	if (!methodID) {
		methodID = env->GetMethodID(VideoViewProxy::javaClass, "destroy", "()V");
		if (!methodID) {
...
void VideoViewProxy::pause(const FunctionCallbackInfo<Value>& args)
{
	LOGD(TAG, "pause()");
	Isolate* isolate = args.GetIsolate();
	HandleScope scope(isolate);
	...
	static jmethodID methodID = NULL;
	if (!methodID) {
		methodID = env->GetMethodID(VideoViewProxy::javaClass, "pause", "()V");
		if (!methodID) {
